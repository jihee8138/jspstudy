...
<%--
  ...
  2. request     : HttpServletRequest : 응답 전까지 접근 가능하다. (response 받으면 끝)
  3. session     : HttpSession        : 브라우저 닫기 전까지 접근 가능하다.
  4. application : ServletContext     : application 종료 전까지 접근 가능하다. (서비스 중지 전까지)
 --%>
  
 <%--
  JSP Bind 영역의 우선 순위
  1. 동일한 이름의 속성(Attribute)이 여러 영역에 동시에 존재하는 경우 우선적으로 사용되는 
  2. 높은 순위                         낮은 순위
...
<body>
  ...
  <%
    Map<String, Object> map = Map.of("title", "어린왕자", "author", "생택쥐베리", "price", 20000);
  ...
  <%--
    ...
    2. 파라미터(Parameter)
      response.semdRedirect("/contextPath/URLMapping?number=10");
      ${param.number}, 문자열 타입
   --%>
