...
public class BoardDaoImpl implements BoardDao {
	...
	// SqlSession (Connection/PreparedStatement/ReaultSet 처리) 만드는 SqlSessionFactory 객체 선언
	// (얘가 일당백임)
	...
	public int insertBoard(BoardDto board) {
		SqlSession sqlSession = factory.openSession(false);  // autoVCommit을 하지 않는다. (즉, 내가 커밋한다.)
		int insertCount = sqlSession.insert("com.gdu.prj.dao.board_t.insertBoard", board);
		if(insertCount == 1) {
			sqlSession.commit();  // 작업이 성공했을 때 커밋.
		}
		sqlSession.close();  // 자원반납코드
		return insertCount;
	}
	...
	public List<BoardDto> selectBoardList(Map<String, Object> params) {
		SqlSession sqlSession = factory.openSession();  // select 라 commit의 개념이 없다.
